...
async def group_export_command(plugin, event: AstrMessageEvent, args: str = ""):
    ...
    if not requested_options:
        requested_options = [
            opt
            for opt in all_possible_options
            if opt in plugin.backup_options and opt != "群相册"
        ]
    ...
    try:
        client = event.bot
        logger.info(
            f"收到导出请求: 群号={group_id}, 选项={requested_options}, 原始消息='{event.message_str}'"
        )
        ...
        yield event.plain_result(
            f"正在导出群 {group_id} 的数据: {', '.join(requested_options)}..."
        )
        ...
        if "群相册" in requested_options:
            ...
            deleted_dir = (
                Path(plugin.plugin_data_dir) / str(group_id) / "logs" / "deleted_items"
            )
            ...
            if has_files(album_dir) or has_files(deleted_dir):
                logger.info(
                    f"正在压缩群 {group_id} 的备份目录（包含相册和已删除项目）..."
                )
                zip_buffer = BytesIO()
                    ...
                    if has_files(deleted_dir):
                        for root, dirs, files in os.walk(deleted_dir):
                            for file in files:
                                file_path = Path(root) / file
                                # 在压缩包内存放在 "回收站" 目录下
                                arcname = Path("回收站") / file_path.relative_to(
                                    deleted_dir
                                )
                                zf.write(file_path, arcname)
        ...
        if excel_options:
            ...
            with pd.ExcelWriter(output_buffer, engine="openpyxl") as writer:
                ...
                if "群信息" in requested_options:
                    ...
                    try:
                        if plugin._is_llbot:
                            ...
                            raw_res = await client.get_group_detail_info(
                                group_id=group_id
                            )
                            if (
                                isinstance(raw_res, dict)
                                and raw_res.get("retcode", 0) != 0
                            ):
                ...
                if "群成员" in requested_options:
                    ...
                    try:
                        ...
                        if plugin._is_llbot:
                            ...
                            isinstance(raw_res, dict) and raw_res.get("retcode", 0) != 0
                        ):
                            raise Exception(f"API 响应异常: {raw_res}")
                        members = raw_res
                    except Exception as e:
                ...
                if "群公告" in requested_options:
                    ...
                    if notices:
                        ...
                        for n in notices:
                            ...
                            images = (
                                (msg.get("image") or msg.get("images"))
                                if isinstance(msg, dict)
                                else n.get("images")
                            )
                            if images:
                    ...
                    if essence and isinstance(essence, list):
                        ...
                        for e in essence:
                            processed_essence.append(
                                {
                                    "发送者": e.get("sender_id"),
                                    "设精时间": plugin._format_timestamp(
                                        e.get("operator_time")
                                    ),
                                    "内容": plugin._format_essence_content(
                                        e.get("content", [])
                                    ),
                ...
                if "群荣誉" in requested_options:
                    honors = {}
                    try:
                        raw_res = await client.get_group_honor_info(
                            ...
                            processed_albums.append(
                                {
                                    ...
                                    "创建时间": plugin._format_timestamp(
                                        a.get("create_time")
                                    ),
                                    "修改时间": plugin._format_timestamp(
                                        a.get("modify_time")
                                    ),
                                }
                            )
                ...
                if archive_file.exists():
                    try:
                        ...
                        for item_type, items in archive.items():
                            ...
                            if (
                                item_type == "notices"
                                and "群公告" not in requested_options
                            ):
                                continue
                            if (
                                item_type == "essence"
                                and "群精华" not in requested_options
                            ):
                                continue
                            if (
                                item_type in ["albums", "media"]
                                and "群相册" not in requested_options
                            ):
                                continue
        ...
        if excel_base64:
            ...
            if (
                event.message_obj.type == MessageType.GROUP_MESSAGE
                and current_context_group_id
            ):
                await client.upload_group_file(
                    group_id=int(current_context_group_id),
                    file=f"base64://{excel_base64}",
                    name=excel_name,
                )
            else:
        ...
        if zip_base64:
            zip_name = (
                f"群{group_id}_群相册_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip"
            )
            current_context_group_id = event.get_group_id()
            if (
                event.message_obj.type == MessageType.GROUP_MESSAGE
